...
def homequit():
    compare_click('./Target/wqmt/homequit.png',sleepn=5,success="点击返回home",fail="")
...
def raidriver(): # 绣河
    compare_click('./Target/wqmt/fuben1.png', sleepn=4, success="开始绣河任务，打开副本界面"+get_time())
    adb_screenshot()
    adb_click_percent(0.17, 0.92, sleepn=3)
    adb_screenshot()
    compare_click('./Target/wqmt/fuben2.png', sleepn=2, success="打开记忆风暴"+get_time()) 
    adb_screenshot()
    adb_click_percent(0.835, 0.682, sleepn=2) # 选择第四关
    adb_screenshot()
    compare_click('./Target/wqmt/fubensaodang.png', sleepn=2, success="点击连续扫荡"+get_time()) 
    compare_click('./Target/wqmt/fubensaodangkaishi.png', sleepn=6, success="点击开始"+get_time()) 
    compare_click('./Target/wqmt/done.png', sleepn=2, success="点击完成"+get_time()) 
    compare_click('./Target/wqmt/cancell.png', sleepn=2, success="次数用光，取消扫荡"+get_time()) 
    topquit()
    [homequit() for i in range(2)]
    adb_screenshot()
    put_text("绣河完成"+get_time())

def raid11(): # 刷11章
    compare_click('./Target/wqmt/fuben1.png', sleepn=4, success="开始raid任务，打开副本界面"+get_time())
    compare_click('./Target/wqmt/fuben3-11.png', sleepn=2, success="打开11章"+get_time())
    ...
    compare_click('./Target/wqmt/fubensaodangkaishi.png', sleepn=12, success="点击开始"+get_time())
    compare_click('./Target/wqmt/done.png', sleepn=2, success="点击完成"+get_time())
    topquit()
    [homequit() for i in range(2)]
    adb_screenshot()
    put_text("11章完成"+get_time())

def raiddark():## 深井
    compare_click('./Target/wqmt/fuben1.png', sleepn=4, success="开始深井任务，打开副本界面"+get_time())
    adb_click_percent(0.837, 0.891, sleepn=2) # 内海
    adb_click_percent(0.193, 0.523, sleepn=2) # 浊暗之井
    while True:
        center = compare_click('./Target/wqmt/fuben4.png', sleepn=2, success="找到乐园"+get_time())
        if center:
            adb_click_percent(0.587, 0.86,sleepn=3) # 点击扫荡
            topquit()
            homequit()
            adb_screenshot()
            put_text("深井完成"+get_time())
            break
        else:
